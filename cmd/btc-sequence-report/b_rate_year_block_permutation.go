...
// init appends bounded calendar-year diagnostics for the observed 2017-2019
// versus 2020-2022 B-incidence shift. They treat calendar years as fixed blocks
// and change no detector or trading rule.
func init() {
	...
	for i := 0; i < 4; i++ {
		for j := i + 1; j < 5; j++ {
			for k := j + 1; k < 6; k++ {
				...
				total++
				if math.Abs(d)+1e-12 >= math.Abs(observed) {
					asExtreme++
				}
				if d < minDelta {
					minDelta = d
	...
	fmt.Println()
	fmt.Println("BTC 15M B-rate contiguous 3-year window diagnostic (DESCRIPTIVE; frozen rules unchanged):")
	fmt.Println("Slides the same equal-weight three-calendar-year B-incidence summary one year at a time across 2017-2022. This checks boundary sensitivity of the fixed-era contrast; no outcome filter, threshold tuning, or rule change.")
	windowRates := make([]float64, 0, 4)
	for start := 0; start <= len(years)-3; start++ {
		window := []int{start, start + 1, start + 2}
		r := mean(window)
		windowRates = append(windowRates, r)
		fmt.Printf("%d-%d year-balanced B incidence: %.1f%%\n", years[start], years[start+2], r)
	}
	fmt.Printf("Adjacent window changes: 2018-2020 minus 2017-2019 %+.1f pp | 2019-2021 minus 2018-2020 %+.1f pp | 2020-2022 minus 2019-2021 %+.1f pp\n",
		windowRates[1]-windowRates[0], windowRates[2]-windowRates[1], windowRates[3]-windowRates[2])
	fmt.Println("Interpretation: use the path to judge whether the era shift is gradual or depends strongly on the 2019/2020 boundary; this is descriptive only.")
}
