...
// init appends one bounded leave-one-trade-out jackknife for the fixed
// 10-18 UTC frozen-trade block. It tests whether that later session result is
...
func init() {
	...
	fmt.Println()
	fmt.Println("BTC 15M UTC 10-18 single-trade jackknife (DESCRIPTIVE; frozen rules unchanged):")
	fmt.Println("Within the fixed 10-18 UTC entry block only, removes each frozen trade once and recomputes remaining NetR/PF. This tests single-observation dependence; it does not create a trade/session filter.")
