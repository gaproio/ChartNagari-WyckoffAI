...
// fixed 10-18 UTC frozen-trade block highlighted by the boundary perturbation.
// It re-prices the exact same frozen trades at baseline, 1.5x, 2x and 3x the
// baseline round-trip friction inferred from gross-vs-net R. Detector, entry,
// stop, target, hold and confirmation rules are unchanged; this is not a
// session or execution-rule optimization.
func init() {
	...
	for s.Scan() {
		...
		if err0 != nil || err1 != nil || err2 != nil || ts.Hour() < 10 || ts.Hour() >= 18 {
			continue
		}
	...
	fmt.Println()
	fmt.Println("BTC 15M UTC 10-18 transaction-cost stress (DESCRIPTIVE; frozen rules unchanged):")
	fmt.Println("Exact same 10-18 UTC frozen trades re-priced at fixed multiples of baseline friction. Baseline cost is inferred trade-by-trade as grossR-netR; no signals, fills, stops, targets or holding rules change.")
