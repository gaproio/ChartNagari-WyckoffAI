...
// shifted 10-18 UTC frozen-trade block highlighted by the fixed boundary
// perturbation. It tests whether that window's net-R contribution is dominated
// by a single year. No detector, confirmation, execution, stop, target,
// holding-period, cost, or session-filter rule changes.
func init() {
	...
	fmt.Println()
	fmt.Println("BTC 15M UTC 10-18 leave-one-year-out concentration stress (DESCRIPTIVE; frozen rules unchanged):")
	fmt.Println("Within the fixed 10-18 UTC entry block only, removes each represented calendar year in turn and recomputes remaining NetR. This tests whether the later boundary-perturbation result depends on one year; it does not create a year/session filter.")
